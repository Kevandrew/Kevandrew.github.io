  ...
  const tasksCombined = thread
    ? thread.messages.flatMap((message) =>
        message.aiOutput && message.aiOutput["Tasks Extracted"]
          ? message.aiOutput["Tasks Extracted"]
          : []
      )
...
export const EmailDetailThreaded: React.FC<EmailDetailThreadedProps> = ({
  ...
  return (
    // Adding `relative` and `h-full` helps the sticky children behave correctly.
    <div className="flex flex-col h-full relative bg-slate-50 dark:bg-slate-950 overflow-y-auto">
      {/* Header with sticky positioning */}
      <div className="sticky top-0 z-10 px-4 py-3 bg-white dark:bg-black border-b border-black/5 dark:border-white/5">
        <div className="flex items-center justify-between mb-3">
          <button
            onClick={onBack}
            className="flex items-center gap-2 text-sm text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white transition-colors"
      ...
      {/* Compose Area with sticky positioning */}
      <div className="sticky bottom-0 z-10 p-4 bg-white dark:bg-black border-t border-black/5 dark:border-white/5">
